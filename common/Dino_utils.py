...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, warmup_teacher_temp, teacher_temp,
                 ...
                 center_momentum=0.9, device='cpu'):
        super().__init__()
        self.student_temp = student_temp
        self.center_momentum = torch.as_tensor(center_momentum)
        self.ncrops = ncrops
    ...
    def forward(self, student_output, teacher_output, epoch, labels=None, weights=None):
        """
        ...
        teacher_out = F.softmax((teacher_output - self.center.to(teacher_output)) /
                                torch.as_tensor(temp).to(teacher_output), dim=-1)
        teacher_out = teacher_out.detach().chunk(2)     # EDIT

        total_loss = 0
        n_loss_terms = 0
        for iq, q in enumerate(teacher_out):
            label = labels[iq].argmax(-1)
    ...
    def update_center(self, teacher_output):
        ...
        # ema update
        self.center = self.center * self.center_momentum.to(teacher_output) + \
                      batch_center * (1 - self.center_momentum.to(teacher_output))
